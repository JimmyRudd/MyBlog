...
4. 执行dx时最好在<要处理的类文件的路径>,因为<要处理的类文件的路径>必须和包路径一致（即com.a.b.c.class要对应com/a/b/c.class）  
...
####注意事项  
1. 必須先使用aapt生成對應的R.java文件（并将其放到需要编译的源文件目录中AndroidManifest.XML中声明的默认包路径下。可以将源文件和aapt生成的R文件按照对应包路径放到一个新建目录下）       
2. -classpath该选项可以引入需要引入的第三方库jar  
